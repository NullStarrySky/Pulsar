            ...
            <AccordionItem :value="rule.id">
              ...
              <AccordionContent class="p-4 border rounded-b-md">
                <div class="space-y-4">
                  ...
                  >
                    <div class="space-y-1">
                      <Label :for="`find-regex-${rule.id}`">查找正则</Label>
                      <Input
                    ...
                    <div class="space-y-1">
                      <Label :for="`replace-string-${rule.id}`">替换到</Label>
                      <Input
                  ...
                  <div class="flex items-center gap-8">
                    <div class="w-full space-y-1">
                      <Label :for="`apply-on-${rule.id}`">应用于</Label>
                      <Select v-model="rule.applyOn">
                        <SelectTrigger :id="`apply-on-${rule.id}`">
                          <SelectValue placeholder="Select when to apply" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="rendering">渲染</SelectItem>
                          <SelectItem value="generating">生成</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
